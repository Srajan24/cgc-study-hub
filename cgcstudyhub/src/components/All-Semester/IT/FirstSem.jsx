...
import { ITSem1Subjects } from "../../../Constants/AI";
        ...
        >
          ...
          <div className="w-full max-w-4xl space-y-8">
            <Suspense fallback={<div className="text-white">Loading Notes...</div>}>
              {[...ITSem1Subjects.filter(sub => sub.subject === "MST & PTU PAPERS"),...sem2Subjects, ...semester1Subjects.filter(sub => sub.subject === "Mathematics-I")]
...
const SubjectBlock = ({ subject, delay }) => {
  ...
  const teachersNotes = subject.units.filter((unit) =>
    unit.name.toLowerCase().includes("teachers notes") || 
    (unit.name.toLowerCase().includes("teacher") && unit.name.toLowerCase().includes("notes"))
  );
  ...
  const others = subject.units.filter(
    (unit) =>
      ...
      !unit.name.toLowerCase().includes("teachers notes") &&
      !unit.name.toLowerCase().includes("teacher") &&
      !unit.name.toLowerCase().includes("notes")
  );
        ...
        <div className="mt-6">
          ...
          {isNotesOpen && (
            ...
            {teachersNotes.length > 0 && (
              <div className="mt-6">
                <button
                  onClick={() => setIsTeacherNotesOpen(!isTeacherNotesOpen)}
                  className="flex items-center justify-between text-left text-black dark:text-white px-2 py-2 rounded-md hover:bg-white/20 transition"
                  >
                  <span className="text-lg font-semibold">Teacher's Notes</span>
                  {isTeacherNotesOpen ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                </button>
      
                {isTeacherNotesOpen && (
                 <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4 ml-2">
                    {teachersNotes.map((unit, unitIndex) => (
                      <UnitLink key={`teacher-${unitIndex}`} unit={unit} delay={unitIndex * 0.05} />
                    ))}
                  </div>
                )}
              </div>
...
const UnitLink = ({ unit, delay }) => {
  ...
  const getBadgeColor = () => {
    ...
    if (label.includes("Teachers Notes")) return "bg-purple-600";
