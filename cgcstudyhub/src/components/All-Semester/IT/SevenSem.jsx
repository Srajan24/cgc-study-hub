...
import { sem7Subjects , sem6Subjects} from "../../../../Constants";
import { ITsem7subject } from "../../../Constants/courses";
...
const sevensemester = () => {
  ...
  const excludedSubjects = [
    ...
    "AWSN (Adhoc and Wireless Sensor Networks )",
    "NSC (Network Security and Cryptography)" ,
  ];
        ...
        >
          ...
          <div className="w-full max-w-4xl space-y-8">
            <Suspense fallback={<div className="text-white">Loading Notes...</div>}>
              {[...ITsem7subject, ...sem7Subjects.filter(sub => sub.subject === "DMDW (Data Mining and Data Warehousing)"),...sem7Subjects.filter(sub => sub.subject === "R&S (Routing & Switching)"),...sem6Subjects.filter(sub => sub.subject === "AI (Artificial Intelligence)")]
...
const SubjectBlock = ({ subject, delay }) => {
    ...
    const [isHandisopen , setIsHandisopen] = useState(false);
    ...
    const Handis = subject?.units?.filter((unit) =>
        unit.name.toLowerCase().includes("handwritten")
    ) || [];
    ...
    return (
        ...
        >
            ...
            {Handis.length > 0 && (
                <div className="mt-6">
                    <button
                        onClick={() => setIsHandisopen(!isHandisopen)}
                        className="flex items-center justify-between text-left text-black dark:text-white px-2 py-2 rounded-md hover:bg-white/20 transition"
                    >
                        <span className="text-lg font-semibold">Handwritten Notes</span>
                        {isHandisopen ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                    </button>

                    {isHandisopen && (
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4 ml-2">
                            {Handis.map((unit, unitIndex) => (
                                <UnitLink key={unitIndex} unit={unit} delay={unitIndex * 0.05} />
                            ))}
                        </div>
                    )}
...
const UnitLink = ({ unit, delay }) => {
    ...
    const isComingSoon = unit.name.toLowerCase().includes("coming soon");

    const getBadgeColor = () => {
        if (isComingSoon) return "bg-gray-600";
        ...
        if (label.includes("handwritten")) return "bg-cyan-600";
        return "bg-gray-600";
    };
