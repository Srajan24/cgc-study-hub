...
import { sem5Subjects, sem4Subjects } from "../../../../Constants";
import { ITSem5Subjects } from "../../../Constants/AI";

const FifthSemester = () => {
    const navigate = useNavigate();
    const excludedSubjects = ["CN", "ERP", "WT (Elective)", "EXAM PAPERS"]; // add any subjects you don’t want

    const sem5WithUHV = [
        ...sem5Subjects,
        sem4Subjects.find(sub => sub.subject === "UHV") // 👈 only adds COA
    ];
                ...
                >
                    ...
                    <div className="w-full max-w-4xl space-y-8">
                        <Suspense fallback={<div className="text-white">Loading Notes...</div>}>
                            {[...ITSem5Subjects,...sem5Subjects, ...sem4Subjects.filter(sub => sub.subject === "UHV"), ]
                                .filter(subject => !excludedSubjects.includes(subject.subject)) // 👈 exclusion works here
                                .map((subject, index) => (
...
const UnitLink = ({ unit, delay }) => {
    ...
    const getBadgeColor = () => {
        ...
        if (label.includes("topics")) return "bg-blue-600";
        if (label.includes("mst")) return "bg-blue-600";
        ...
        if (label.includes("handwritten")) return "bg-blue-600";
        if (label.includes("cec")) return "bg-emerald-600";
        if (label.includes("qb")) return "bg-blue-600";
        return "bg-gray-600";
    };
